...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not stats:
        await update.message.reply_text("📭 هنوز هیچ کلیکی ثبت نشده.")
        return

    text = "📊 آمار کلیک فایل‌ها:\n\n"

    for key, count in stats.items():
        title = books.get(key, {}).get("title", "Unknown")
        text += f"📘 {title} ({key}) → {count} کلیک\n"
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc.file_name.lower().endswith(allowed):
        await update.message.reply_text("⚠️ فقط PDF یا ویدیو قبول میشه.")
        return
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📝 عنوان رو بنویس:")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "waiting_title":
        ...
        await update.message.reply_text("📄 توضیح کوتاه بنویس (یا /skip):")
...
def main():
    ...
    app.add_handler(CommandHandler("stats", stats_command))  # NEW
